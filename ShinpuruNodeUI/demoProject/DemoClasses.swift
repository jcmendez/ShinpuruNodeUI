...
class DemoNode: SNNode
{
  ...
  {
    guard let ins = inputs else { return DemoNodeValue.number(0) }
    if index >= ins.count || ins[index] == nil || ins[index]?.demoNode == nil
    {
      return DemoNodeValue.number(0)
    }
    else if let value = ins[index]?.demoNode?.value
    {
      return value
    }
    else
